<script lang="ts">
	...
	$: Controls = {
		add: {
			...
			action: () => {
				if (browser) goto(`${base}/${$page.route.id}/controls`)
			}
		},
		remove: {
			text: 'Turn off controls',
			action: () => {
				if (browser) goto(`${base}/${$page.route.id?.replace('/controls', '')}`)
			}
		}
	}
	...
	$: RenderStyle = {
		twoDim: {
			...
			action: () => {
				if (browser) goto(`${base}/${$page.route.id?.replace('3d', '2d')}`)
			}
		},
		threeDim: {
			text: 'Render in 3D (WIP / not recommended)',
			action: () => {
				if (browser) goto(`${base}/${$page.route.id?.replace('2d', '3d')}`)
			}
		}
	}
